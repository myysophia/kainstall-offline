...
function archive_images() {
  ...
  echo "docker images --format '{{.Repository}}:{{.Tag}}' | grep coredns/coredns | awk -F'k8s.gcr.io/coredns/' '{print "docker tag " $0 " " "k8s.gcr.io/" $2 " && docker rmi " $0 }' | bash"
  ...
  echo "docker images --format '{{.Repository}}:{{.Tag}}' | grep k8s.gcr.io | awk -F'k8s.gcr.io/' '{print "docker tag " $0 " registry.cn-hangzhou.aliyuncs.com/kainstall/" $2}' | bash"
  ...
  echo "docker rmi $(docker images --format '{{.Repository}}:{{.Tag}}' | grep k8s.gcr.io)"
